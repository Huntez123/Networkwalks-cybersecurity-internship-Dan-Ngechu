...
A custom VirtualBox NAT Network named `NatNetwork` was configured using:
```text
Network: 10.0.0.0/24
Gateway: 10.0.0.1
DHCP was disabled so that the Kali Linux machine could use the required static address:
...
ping -c 3 8.8.8.8
The Internet connectivity test completed successfully.
...
Troubleshooting
During the static IP configuration, Kali Linux initially failed to activate the 10.0.0.2/24 configuration.
The Networkwalks troubleshooting guidance for newer VirtualBox/Kali configurations was followed.
...
Evidence
Screenshots documenting the lab setup are maintained in the:
screenshots/
directory.
...
Shared folder configuration
Shared folder functionality test
Kali Linux VM snapshot
Learning Outcomes
...
Future exercises may therefore expand the laboratory with additional virtual machines for cybersecurity and ethical hacking practice.
